...
const {evaluateUGCForm3Engineering} = require("../evaluation/UGCform3EvaluationEngineering");
const {evaluateUGCForm3Language}= require("../evaluation/UGCform3EvaluationLanguage");
...
router.post("/submitFormData", async (req, res) => {
  try {
    ...
    if (formType === "Form1") {
      ...
      totalScore = evaluateUGCForm3Engineering({ userId: user._id, formData }).totalScore;
    } else if (formType === "UGCForm3Language") {
      totalScore = evaluateUGCForm3Language({ userId: user._id, formData }).totalScore;
    } else {
    ...
    console.log(
      `✅ Form data saved for ${email} (${formType}) with totalScore: ${totalScore}`
    );
